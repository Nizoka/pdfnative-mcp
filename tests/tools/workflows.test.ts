...
// Ported from pdfnative-cli's tests/tools/workflows.test.ts (1.5.0, itself from
// pdfnative 1.8.0), with the Windows job this repository keeps.
...
describe('publish.yml', () => {
    ...
    it('runs the publish gate with --require-all after veraPDF, and lists no gate step by hand', () => {
        ...
        expect([verapdf, gate, pack, pub].every((i) => i >= 0)).toBe(true);
        expect(verapdf).toBeLessThan(gate);
        expect(gate).toBeLessThan(pack);
        expect(pack).toBeLessThan(pub);
        for (const hand of ['npm run validate:pdfa', 'npm run corpus:pdfa', 'npm run verify:samples', 'npm run test:coverage', 'npm run typecheck:all', 'npm run verify:tool-shape']) {
    ...
    it('has an attest job with exactly three permissions that attests the tarball and the SBOM', () => {
        ...
        expect(body).toMatch(/pdfnative-mcp-\$\{VERSION\}\.tgz/);
...
describe('package.json publish settings', () => {
    ...
    it('keeps exactly three runtime dependencies', () => {
        expect(Object.keys(pkg.dependencies).sort()).toEqual(['@modelcontextprotocol/server', 'pdfnative', 'zod']);
    });
...
describe('sample-regression.yml', () => {
    ...
    it('builds the server before generating and verifying', () => {
