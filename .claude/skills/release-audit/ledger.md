...
|---|---|---|---|---|---|---|
| A-03 | major | "`pdfx: 'pdfx4'` writes a /GTS_PDFX output intent" | `npx vitest run tests/pdfx.test.ts -t "GTS_PDFX"` | 1 passed | 1 passed | holds |
| B-07 | blocker | the error table documents `PDF_X_COMPLIANCE_VIOLATION` | `npx vitest run tests/error-codes.test.ts` | missing row | documented | finding |
...
| … | stamp | justification |
|---|---|---|
| … | CONFIRMED | re-ran the command; the code is thrown by `escalate()` in src/diagnostics.ts and has no row in docs/AGENT_CONTRACT.md §6 |
| … | DOWNGRADED → minor | real, but `error-parity` fails the gate on it — cannot ship silently |
| … | REJECTED | the input is documented in the tool description and in the `print_ready` prompt |
