...
- No `$ref` / `$defs` and no `$schema` keyword in input schemas (host compatibility); shared fragments are therefore inlined in every tool — keep their descriptions terse (`tools/list` ≈ 303 kB) and put the long form in a prompt or a guide.
