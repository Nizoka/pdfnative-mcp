...
specification, and grows the catalogue to **28 tools**: full engine coverage (all
13 document block kinds in `generate_basic_pdf`, layout options and build-time
`encrypt` on the document tools, image watermarks, `inspect_pdf` annotations, the
read-only `inspect_layout` pagination preview), the full PAdES baseline ladder
(`sign_pdf` timestamps, `add_ltv`, `timestamp_pdf`), print production, charts v2,
PDF/A conformance diagnostics with `embedFonts`, `update_metadata`, the SDK v2
transport with legacy fallback, and a network charter that keeps the server offline
by default (operator-configured TSA / OCSP / CRL endpoints only). Default tool
outputs stay byte-identical except where pdfnative 1.7.0 corrected previously-wrong
output (see `release-notes/v1.6.0.md` → Upgrade); a superset gate against the
published 1.5.0 catalogue proves nothing was removed or narrowed.
...
- **HTTP transport** — opt-in bearer token `PDFNATIVE_MCP_HTTP_TOKEN` (401 + RFC 6750 `WWW-Authenticate` without it); the `Origin` port must equal the server port; the default loopback endpoint has no authentication and SECURITY.md now lists that explicitly. Strict input schemas reject unknown keys; base64 / DER payloads are sanity-checked at the boundary; keep-alive connections no longer leak socket listeners; `sign_pdf` output and any `encrypt` call are never written to the response cache.
- **Resource bounds** — operator-set decompression cap (`PDFNATIVE_MCP_MAX_INFLATE_BYTES`, zip-bomb guard); 24 MiB decoded-image budget per call; PNG variants the engine cannot decode are rejected at the boundary; `link` URLs restricted to `http:` / `https:` / `mailto:` without control characters; SVG never fetches anything.
