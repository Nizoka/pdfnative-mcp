/**
 * A deliberately small JSON Schema (draft-07 subset) validator.
 *
 * It exists for one job: holding `server.json` to the vendored MCP registry
 * schema inside the hermetic gate, with no network and no new dependency.
 * It is NOT a general-purpose validator, and it is built so that this
 * limitation cannot turn into a silent pass: a keyword it does not implement
 * is reported as a failure, so a schema revision that starts relying on one
 * makes the check go red instead of vacuous.
 *
 * Annotations (`title`, `description`, `default`, `example(s)`, `$comment`,
 * `$id`, `$schema`) are accepted and ignored; `format` is an annotation in
 * draft-07 and is treated as one.
 */

export type Json = null | boolean | number | string | readonly Json[] | { readonly [key: string]: Json };
type Schema = { readonly [key: string]: Json };

const ANNOTATIONS = new Set(['$comment', '$id', '$schema', 'title', 'description', 'default', 'example', 'examples', 'format', 'definitions']);
const IMPLEMENTED = new Set([
    '$ref', 'type', 'enum', 'const', 'required', 'properties', 'additionalProperties', 'items',
    'minLength', 'maxLength', 'pattern', 'minItems', 'maxItems', 'minimum', 'maximum', 'uniqueItems',
...
export function validate(value: Json, schema: Schema, root: Schema = schema, at = '$'): string[] {
    ...
    if (typeof value === 'string') {
        // Length in code points, as the specification counts it.
        const length = Array.from(value).length;
        const min = schema['minLength'];
        const max = schema['maxLength'];
        const pattern = schema['pattern'];
        if (typeof min === 'number' && length < min) errors.push(`${at}: shorter than minLength ${min}`);
        if (typeof max === 'number' && length > max) errors.push(`${at}: longer than maxLength ${max} (${length})`);
    ...
    if (typeof value === 'number') {
        ...
        if (typeof min === 'number' && value < min) errors.push(`${at}: below minimum ${min}`);
        if (typeof max === 'number' && value > max) errors.push(`${at}: above maximum ${max}`);
    }
    ...
    if (Array.isArray(value)) {
        ...
        if (typeof min === 'number' && value.length < min) errors.push(`${at}: fewer than minItems ${min}`);
        if (typeof max === 'number' && value.length > max) errors.push(`${at}: more than maxItems ${max}`);
        if (schema['uniqueItems'] === true && new Set(value.map((v) => JSON.stringify(v))).size !== value.length) {
            errors.push(`${at}: items are not unique`);
        }
        if (isObject(items)) value.forEach((v, i) => errors.push(...validate(v, items, root, `${at}[${i}]`)));
        else if (items !== undefined) errors.push(`${at}: tuple-form "items" is not implemented by json-schema-lite`);
    }
    ...
    if (anyOf !== null) {
        ...
        if (!results.some((r) => r.length === 0)) {
            errors.push(`${at}: matches none of the ${anyOf.length} anyOf branches (closest: ${results.sort((a, b) => a.length - b.length)[0]?.[0] ?? 'n/a'})`);
        }
    }
    const oneOf = branches('oneOf');
    if (oneOf !== null) {
        const passing = oneOf.filter((s) => validate(value, s, root, at).length === 0).length;
        if (passing !== 1) errors.push(`${at}: matches ${passing} of the ${oneOf.length} oneOf branches, expected exactly 1`);
    }
