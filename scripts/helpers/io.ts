 ...
 * verifiers. Ported from pdfnative-cli's scripts/helpers/io.ts; the
 * generators here drive the BUILT server (`dist/server.js`) through the same
 * `tools/call` handler an MCP host reaches, so files are written by the
 * script from the returned bytes.
 */
 ...
 *
 * The server is told the instant twice, on purpose: per call (`creationDate`,
 * `signingTime`, `modDate` wherever the tool's live input schema declares
 * them — see `pinArgs()` in ./server.ts) and process-wide (the engine's
 * `setDefaultCreationDate()`), so a generator that forgets one still produces
 * the baseline bytes.
 */
