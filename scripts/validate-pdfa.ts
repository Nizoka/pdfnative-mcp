#!/usr/bin/env tsx
/**
 * pdfnative-mcp — veraPDF batch validation runner (v1.7.0)
 * =========================================================
 ...
 *   - veraPDF is an external tool: pdfnative-mcp adds no runtime
 *     dependencies and never bundles a validator.
 *
 * Environment:
 ...
 *          conformance verdict.
 *   SKIP   no PDF/A claim (PDF/X and page-tree outputs: merge_pdfs, extract_pages) — never sent to veraPDF.
 *
 * Exit codes:
...
function printMissingVeraPdfHelp(): void {
    for (const l of [
        ...
        '',
        '  pdfnative-mcp does not bundle a validator (no new runtime dependency).',
        ...
        '',
        '  See CONTRIBUTING.md §PDF/A validation.',
    ]) log(l);
}
...
function main(): number {
    ...
    if (unlisted.length > 0) {
        note(`Note: ${unlisted.length} PDF(s) in test-output/pdfa/ are not in manifest.json and are ignored: ${unlisted.join(', ')}`);
    }
    if (listed.length === 0) {
    ...
    if (declared !== null && declared !== claimed.length) {
        log(`Coverage canary: ${claimed.length} file(s) claim PDF/A but docs/assets/ecosystem.json declares pdfaSamples: ${declared}. Update the manifest in the same change as the corpus.`);
        return EXIT_CONFORMANCE;
    }
    const negatives = claimed.filter((c) => !c.expectCompliant);
    note(`Corpus: ${listed.length} file(s) in manifest.json — ${claimed.length} claim PDF/A (${negatives.length} negative canar${negatives.length === 1 ? 'y' : 'ies'}), ${skipped.length} file(s) without a claim (as expected).`);
    ...
    note(`Validating ${claimed.length} file(s)…`);
    ...
    const showRules = (rules: readonly string[]): void => {
        ...
        if (rules.length > shown.length) say(`        … (${rules.length - shown.length} more)`);
    ...
    for (const { file, name, claim, expectCompliant } of claimed) {
        ...
        const parsed = r.error
            ...
            : r.stdout.trim().length === 0
                ? { kind: 'infra' as const, detail: `empty stdout (exit ${r.status})` }
    ...
    say('');
    say(`Summary: ${counts.PASS} PASS, ${counts.XFAIL} XFAIL, ${counts.FAIL} FAIL, ${counts.XPASS} XPASS, ${counts.INFRA} INFRA, ${skipped.length} SKIP (of ${claimed.length} validated).`);
