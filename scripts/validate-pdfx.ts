#!/usr/bin/env tsx
/**
 * pdfnative-mcp — PDF/X-4 corpus validation (v1.7.0)
 * ===================================================
...
function main(): number {
    ...
    if (canaryFailures > 0) {
        log(`\nCoverage canary failed for ${canaryFailures} of ${entries.length} file(s).`);
        return PDFX_EXIT_CONFORMANCE;
    }
    const declared = declaredPdfxSamples();
    if (declared !== null && declared !== claimed.length) {
        log(`Coverage canary: ${claimed.length} file(s) claim PDF/X but docs/assets/ecosystem.json declares pdfxSamples: ${declared}.`);
        return PDFX_EXIT_CONFORMANCE;
    }
    ...
    note(`Corpus: ${claimed.length} file(s) claim PDF/X (${negatives} negative canar${negatives === 1 ? 'y' : 'ies'}); validator: pdfnative validatePdfX() (structural, ISO 15930-7).`);
    ...
    const showErrors = (errors: readonly string[]): void => {
        ...
        if (errors.length > shown.length) say(`        … (${errors.length - shown.length} more)`);
    };
    ...
    for (const c of claimed) {
        ...
        switch (outcome) {
            case 'PASS':
                if (!quiet) {
                    say(`  PASS   [${c.version}]  ${rel}${result.warnings.length > 0 ? `  (${result.warnings.length} warning(s))` : ''}`);
                }
                break;
            case 'XFAIL':
    ...
    say('');
    say(`Summary: ${counts.PASS} PASS, ${counts.XFAIL} XFAIL, ${counts.FAIL} FAIL, ${counts.XPASS} XPASS (of ${claimed.length} validated).`);
