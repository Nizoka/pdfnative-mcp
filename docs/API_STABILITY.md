...
Current value: **`1.1.0`** (stable, since pdfnative-mcp 1.1.0).
...
All 13 tools shipped in pdfnative-mcp 1.1.0 are at `_meta.apiVersion = '1.1.0'` and are considered **stable**:
...
> **v1.1.0 minor bump rationale:** added the `validate_pdf` tool and six new `add_international_text` `lang` enum values (`te`, `si`, `bo`, `km`, `my`, `am`) — both additive per §3. `add_international_text` also began NFC-normalising input, which can change generated bytes for decomposed inputs; this is a rendering-coverage improvement, not a schema-contract change.
