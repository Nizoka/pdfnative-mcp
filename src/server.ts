...
const TOOLS: readonly ToolDefinition[] = [
    ...
    },
    {
        ...
        description:
            "Apply a PAdES-compatible CMS digital signature to a PDF. Since v1.0.0 you can sign ANY PDF in ONE call — autoInjectPlaceholder defaults to true, so you do NOT need to run prepare_signature_placeholder first unless you want to customize the placeholder appearance. Supports RSA-SHA256 and ECDSA-SHA256 (P-256). Required inputs: pdfBase64, algorithm, certDerBase64, plus EITHER rsaKeyPkcs1DerBase64 (when algorithm='rsa-sha256') OR ecPrivateScalarHex / ecPrivateKeyDerBase64 (when algorithm='ecdsa-sha256'). To convert PEM keys to DER base64: `openssl pkey -in key.pem -outform DER | base64 -w0`. To convert a PEM X.509 cert: `openssl x509 -in cert.pem -outform DER | base64 -w0`. After signing, call verify_pdf to confirm validity. v1.6.0 (pdfnative 1.7): `profile: 'pades'` (ETSI EN 319 142-1 baseline, ESS signing-certificate-v2, ETSI.CAdES.detached), `algorithm` rsa-sha384 / rsa-sha512, `certChainDerBase64` (intermediates), `timestamp: true` for a PAdES B-T RFC 3161 signature timestamp through the operator-configured TSA (PDFNATIVE_MCP_TSA_URL — TSA_NOT_CONFIGURED otherwise, no network call without it), `fieldName` to pick a placeholder (PLACEHOLDER_AMBIGUOUS when several are unsigned) and `allowMultiple: true` to add a further signature. Signer metadata is baked into the placeholder this call injects. Continue the ladder with add_ltv (B-LT) and timestamp_pdf (B-LTA).",
